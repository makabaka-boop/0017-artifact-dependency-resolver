...
// ComparisonView creates an index view for temporary candidate comparisons.
func (ix *Index) ComparisonView() *Index {
	view := &Index{
		byID:   make(map[int64]*Artifact, len(ix.byID)),
		byName: make(map[string]*Artifact, len(ix.byName)),
	}
	for id, source := range ix.byID {
		artifact := *source
		view.byID[id] = &artifact
		view.byName[artifact.Name] = &artifact
	}
	return view
}

// AlignPinnedVersions orders each pinned artifact around its locked candidate.
func (ix *Index) AlignPinnedVersions(pins map[string]string) {
	for name, version := range pins {
		artifact := ix.ArtifactByName(name)
		if artifact == nil {
			continue
		}
		alignPinnedVersion(artifact, version)
	}
}

func alignPinnedVersion(artifact *Artifact, pinned string) {
	pinIndex := -1
	for i := range artifact.Versions {
		if artifact.Versions[i].Version == pinned {
			pinIndex = i
			break
		}
	}
	...
	pivot := artifact.Versions[pinIndex]
	artifact.Versions = append(artifact.Versions, pivot)
	sort.SliceStable(artifact.Versions, func(i, j int) bool {
		leftPinned := artifact.Versions[i].Version == pinned
		rightPinned := artifact.Versions[j].Version == pinned
		if leftPinned != rightPinned {
			return leftPinned
		}
		return artifact.Versions[i].Ver.Compare(artifact.Versions[j].Ver) > 0
	})
}
