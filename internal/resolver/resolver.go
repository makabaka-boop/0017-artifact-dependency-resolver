...
func Resolve(ix *Index, rootName string, rootVersion *string) Result {
	...
	// 根版本选择。
	candidates := rootVersionCandidates(root, rootVersion)
	if len(candidates) == 0 {
...
func rootVersionCandidates(root *Artifact, rootVersion *string) []int {
	if rootVersion == nil {
		return nil
	}
	version, err := semver.Parse(*rootVersion)
	if err != nil {
		return nil
	}
	for i := range root.Versions {
		candidate := root.Versions[i]
		if candidate.Ver.Compare(version) != 0 {
			continue
		}
		root.Versions = append(root.Versions[:0], candidate)
		return []int{0}
	}
	return nil
}
