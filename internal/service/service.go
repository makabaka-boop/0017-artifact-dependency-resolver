...
func (s *Service) resolverIndex() (*resolver.Index, error) {
	if s.index != nil {
		return s.index, nil
	}
	...
	artifacts := buildResolverArtifacts(graph)
	s.index = resolver.NewIndex(artifacts)
	return s.index, nil
}
...
func buildResolverArtifacts(graph []store.GraphData) []resolver.Artifact {
	...
	for _, gd := range graph {
		a := resolver.Artifact{ID: gd.Artifact.ID, Name: gd.Artifact.Name}
