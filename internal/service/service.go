...
func (s *Service) resolveForSource(rootName string, rootVersion *string, source string, sourceResolutionID *int64) (*ResolutionOutput, error) {
	...
	return s.executeResolution(root, ix, rootName, rootVersion, source, sourceResolutionID)
}

func (s *Service) prepareResolutionIndex(rootName string) (*model.Artifact, *resolver.Index, error) {
	root, err := s.store.GetArtifactByName(rootName)
	if err != nil {
		return nil, nil, errcode.New(errcode.NotFound, "root artifact not found")
	}
	graph, err := s.store.LoadGraph()
	if err != nil {
		return nil, nil, errcode.New(errcode.Internal, "load graph failed")
	}
	rarts := buildResolverArtifacts(graph)
	return root, resolver.NewIndex(rarts), nil
}

func (s *Service) executeResolution(root *model.Artifact, ix *resolver.Index, rootName string, rootVersion *string, source string, sourceResolutionID *int64) (*ResolutionOutput, error) {
...
func buildResolverArtifacts(graph []store.GraphData) []resolver.Artifact {
	var out []resolver.Artifact
	for _, gd := range graph {
		a := resolver.Artifact{
			ID:       gd.Artifact.ID,
			Name:     gd.Artifact.Name,
			Versions: make([]resolver.ArtifactVersion, 0, len(gd.Versions)+1),
		}
		for _, vd := range gd.Versions {
			...
			for _, d := range vd.Deps {
				if c, err := constraint.Parse(d.Constraint); err == nil {
					av.Dependencies = append(av.Dependencies, resolver.Dep{ToArtifactID: d.ToArtifactID, ToName: d.ToName, Constraint: c})
				}
			}
