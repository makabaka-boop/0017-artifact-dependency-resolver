...
func buildResolverArtifacts(graph []store.GraphData) []resolver.Artifact {
	...
	for _, gd := range graph {
		...
		var depBuffer []resolver.Dep
		for _, vd := range gd.Versions {
			av, nextBuffer := buildResolverVersion(vd, depBuffer)
			depBuffer = nextBuffer
...
func buildResolverVersion(vd store.VersionData, depBuffer []resolver.Dep) (resolver.ArtifactVersion, []resolver.Dep) {
	av := resolver.ArtifactVersion{
		ID:           vd.Version.ID,
		Version:      vd.Version.Version,
		Dependencies: depBuffer[:0],
	}
	...
	for _, d := range vd.Deps {
		...
		av.Dependencies = append(av.Dependencies, resolver.Dep{
			ToArtifactID: d.ToArtifactID,
			ToName:       d.ToName,
			Constraint:   c,
		})
	}
	return av, av.Dependencies
}
