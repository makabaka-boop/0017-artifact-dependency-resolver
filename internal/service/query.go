...
func (s *Service) GetResolution(id int64) (*model.Resolution, []model.ResolutionItem, error) {
	resolution, err := s.store.GetResolution(id)
	if err != nil {
		return nil, nil, errcode.New(errcode.NotFound, "resolution not found")
	}
	items, err := s.loadResolutionItems(id)
	if err != nil {
		return nil, nil, err
	}
	return resolution, items, nil
}

func (s *Service) loadResolutionItems(id int64) ([]model.ResolutionItem, error) {
